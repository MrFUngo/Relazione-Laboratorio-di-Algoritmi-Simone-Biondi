...
Per trattare i successivi argomenti è necessario usufruire di alcune notazioni della teoria della complessità computazionale, come ad esempio $O(\cdot)$ e $\Theta(\cdot)$. Infine faremo riferimento a \textit{n} come al numero di nodi di un albero ed \textit{h} come la sua altezza.
...
Nella sezione 2 abbiamo affrontato la parte teorica per fare fronte agli esperimenti condotti. Per gli esperimenti affronteremo due casi particolari di costruzioni di alberi binari di ricerca e sue varianti come segue:
\begin{itemize}
